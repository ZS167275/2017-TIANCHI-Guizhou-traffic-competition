...
training_data = pd.read_table(u'./[新-训练集]gy_contest_traveltime_training_data_second.txt',sep=';')
...
testing_data = pd.read_table(u'./[新-答案模板]gy_contest_result_template.txt',sep='#',header=None)
...
def mode_function(df):
    counts = mode(df)
    return counts[0][0]
# [378]	validation_0-rmse:0.453285	validation_0-mape:0.291181
...
# train_5 = feature_data.loc[(feature_data.time_interval_month == 5)&(feature_data.time_interval_begin_hour==7),: ]
# for i in [58,48,38,28,18,8,0]:
...
#                                 ('mode_%d' % (i), mode_function), ('std_%d' % (i), np.std), ('max_%d' % (i), np.max),('min_%d' % (i), np.min)]).reset_index()
#     tmp['std_%d' % (i)] = tmp['std_%d' % (i)].fillna(0)
#     train_5 = pd.merge(train_5,tmp,on=['link_ID','time_interval_day'],how='left')
...
# train_3 = feature_data.loc[(feature_data.time_interval_month == 3)&(feature_data.time_interval_begin_hour==18),: ]
# for i in [58,48,38,28,18,8,0]:
...
#                                 ('mode_%d' % (i), mode_function), ('std_%d' % (i), np.std), ('max_%d' % (i), np.max),('min_%d' % (i), np.min)]).reset_index()
#     tmp['std_%d' % (i)] = tmp['std_%d' % (i)].fillna(0)
#     train_3 = pd.merge(train_3,tmp,on=['link_ID','time_interval_day'],how='left')
print '123'
