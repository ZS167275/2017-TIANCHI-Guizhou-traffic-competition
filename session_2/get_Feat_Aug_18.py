...
# train619 = feature_data.loc[(feature_data.time_interval_month == 6)&(feature_data.time_interval_begin_hour==19),: ]
# for i in [58,48,38,28,18,8,0]:
#     tmp = feature_data.loc[(feature_data.time_interval_month == 6)&(feature_data.time_interval_begin_hour == 18)&(feature_data.time_interval_minutes >= i),:]
#     tmp = tmp.groupby(['link_ID', 'time_interval_day'])[
#             'travel_time'].agg([('mean_%d' % (i), np.mean), ('median_%d' % (i), np.median),
#                                 ('mode_%d' % (i), mode_function), ('std_%d' % (i), np.std), ('max_%d' % (i), np.max),('min_%d' % (i), np.min)]).reset_index()
#     tmp['std_%d' % (i)] = tmp['std_%d' % (i)].fillna(0)
#     train619 = pd.merge(train619,tmp,on=['link_ID','time_interval_day'],how='left')
# 合并训练数据
# train = pd.concat([train517,train518,train519,train419,train617,train68,train619,train418],axis=0)

# train = pd.concat([train517,train518,train519,train617,train68,train418,train419],axis=0)
...
tt[['link_ID','date','time_interval','travel_time']].to_csv('./2017-09-15_18_xgb.txt',sep='#',index=False,header=False)
print tt[['link_ID','date','time_interval','travel_time']].shape
print tt[['link_ID','date','time_interval','travel_time']].isnull().sum()

# [383]   validation_0-rmse:0.443618      validation_0-mape:0.286548
# [392]   validation_0-rmse:0.440936      validation_0-mape:0.286057
# [392]   validation_0-rmse:0.440936      validation_0-mape:0.286057

# 3 4 5 6 7
# [488]   validation_0-rmse:0.443788      validation_0-mape:0.282757